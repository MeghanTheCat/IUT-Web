        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(isset($favorite))
                        <x-nav-link :href="route('weather.weekly', ['city' => $favorite])" :active="request()->fullUrl() == route('weather.weekly', ['city' => $favorite])">
                            <div class="flex items-center">
                                {{ $favorite }}
                                <span class="ml-2 text-yellow-500">
                                    <i class="fas fa-star"></i> <!-- Étoile pleine pour la ville favorite -->
                                </span>
                            </div>
                        </x-nav-link>
                    @endif
                    ...
                    <!-- {{var_dump($savedCities)}} -->
                        @foreach ($savedCities as $query)
                            <x-nav-link :href="route('weather.weekly', ['city' => $query->city])" :active="request()->fullUrl() == route('weather.weekly', ['city' => $query->city])">
                                {{$query->city}}
                                <form action="{{ route("weather.kill", $query->id) }}" method="POST" class="ml-2">
                                    @csrf
                                    @method('DELETE')
