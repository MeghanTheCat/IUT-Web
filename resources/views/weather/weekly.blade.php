    ...
    <div class="container">
        ...
        @else
            <div class="weekly-forecast">
                <a href="{{ route('weather.save', ['city' => $city]) }}">Save</a>
                <a href="{{ route('favorite.add', ['city' => $city]) }}">Add to favorite</a>
                @foreach ($forecast as $day)
                    <div class="day-forecast">
                        <h2>{{ $day['date']->format('m/d/Y') }}</h2>
                        @if(isset($day['icon']))
                            <img src="http://openweathermap.org/img/wn/{{ $day['icon'] }}.png" alt="Icône météo">
                        @else
